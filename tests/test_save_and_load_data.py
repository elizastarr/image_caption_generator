...
def test_load_representations_captions_images_raises_error_stage():
    # Given
    stage = "stage_not_implemented"

    # When
    with pytest.raises(ValueError) as error:
        sld.load_representations_captions_images(stage)

    # Then
    assert "Stage must be either train, val, or test" in str(error.value)


def test_save_representations_captions_images_raises_error_stage(processed_data_100):
    # Given
    ...
    # When
    with pytest.raises(ValueError) as error:
        sld.save_representations_captions_images(
